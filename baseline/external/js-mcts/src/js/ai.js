...
class MonteCarloTreeSearch {
    constructor(game, uctConst, rolloutGreedyProb = 0.7) {
        this.game = game;
        this.uctConst = uctConst;
        this.rolloutGreedyProb = rolloutGreedyProb;
        this.root = new MNode(null, null, this.uctConst);
        this.totalNumOfSimulations = 0;
        this.rolloutShortest = 0;
        this.rolloutWall = 0;
        this.rolloutBack = 0;
    }
    ...
    rollout(node) {
        ...
        while (simulationGame.winner === null && steps < maxSteps) {
            steps += 1;
            ...
            const rolloutRand = Math.random();
            if (rolloutRand < this.rolloutGreedyProb) {
                this.rolloutShortest += 1;
        ...
        if (simulationGame.winner === null) {
            const dist0 = AI.getShortestDistanceToGoalFor(simulationGame.pawn0, simulationGame);
            const dist1 = AI.getShortestDistanceToGoalFor(simulationGame.pawn1, simulationGame);
            if (dist0 === dist1) {
                simulationGame.winner = Math.random() < 0.5 ? simulationGame.pawn0 : simulationGame.pawn1;
            } else {
                simulationGame.winner = dist0 < dist1 ? simulationGame.pawn0 : simulationGame.pawn1;
            }
        }
...
*/
class AI {
    constructor(numOfMCTSSimulations, uctConst, aiDevelopMode = false, forWorker = false, rolloutGreedyProb = 0.7) {
    ...
    chooseNextMove(game, trace = false) {
        const d0 = new Date();
        ...
        if (game.turn < 2) {
            ...
            if (pawnMoveTuple[1] === 0) {
                ...
                const move = [[nextPosition.row, nextPosition.col], null, null];
                if (trace) {
                    return { move: move, trace: { totalSims: 0, children: [] } };
                }
                return move;
            }
        }

        const mcts = new MonteCarloTreeSearch(game, this.uctConst, this.rolloutGreedyProb);
        ...
        if (game.turn < 6 && game.pawnOfNotTurn.position.col === 4 && game.pawnOfNotTurn.position.row === 6 && Math.random() < 0.5) {
            const bestMoves = [
                [null, [5, 3], null], 
                [null, [5, 4], null],
                [null, null, [4, 3]],
        ...
        if (game.turn < 6 && game.pawnOfNotTurn.position.col === 4 && game.pawnOfNotTurn.position.row === 2 && Math.random() < 0.5) {
            const bestMoves = [
                [null, [2, 3], null], 
                [null, [2, 4], null],
                [null, null, [3, 3]],
        ...
        if (trace) {
            const children = mcts.root.children.map((child) => ({
                move: child.move,
            ...
            return {
                move: bestMove,
                trace: {
                    totalSims: mcts.root.numSims,
                    children,
                    rollout: {
                        shortest: mcts.rolloutShortest,
                        wall: mcts.rolloutWall,
                        backward: mcts.rolloutBack,
                    },
                },
            };
        }
