...
class QuoridorNNet(nn.Module):
    def __init__(self, game, args):
        ...
        self.attn_blocks = nn.ModuleList([
            AttnWithLinearBlock(channels, num_heads, args.dropout) for i in range(attn_depth)])
        ...
        self.nan_warn_count = 0
        self.nan_warn_limit = 5
    ...
    def forward(self, s, logits=False):
        ...
        pi_bad = t.isnan(pi).any() or t.isinf(pi).any()
        v_bad = t.isnan(v).any() or t.isinf(v).any()
        if pi_bad or v_bad:
            if self.nan_warn_count < self.nan_warn_limit:
                print("WARNING: NaN/Inf detected in network output; sanitizing.")
                self.nan_warn_count += 1
            elif self.nan_warn_count == self.nan_warn_limit:
                print("WARNING: NaN/Inf detected repeatedly; suppressing further warnings.")
                self.nan_warn_count += 1
        if pi_bad:
            pi = t.nan_to_num(pi, nan=0.0, posinf=1e4, neginf=-1e4)
        if v_bad:
            v = t.nan_to_num(v, nan=0.0, posinf=1.0, neginf=-1.0)
        if (pi == 0.0).all():
            print(pi, v)
            print("x:", x)
            print("s:", s)
