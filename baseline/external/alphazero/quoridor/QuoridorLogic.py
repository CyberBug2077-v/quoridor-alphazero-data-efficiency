...
import sys, os
sys.path.append(os.path.join(os.path.dirname(__file__), "pathFinder-module"))
import pathFinder
...
class Board():
    ...
    def execute_move(self, move, color):
        result = self.action_from_index(move, color)
        if result is None:
            # Invalid move
            raise ValueError(f"Invalid move {move} for color {color}: action_from_index returned None")
        a, (x, y) = result
        # moving
        idx = 0 if color==1 else 1
        x = int(x)
