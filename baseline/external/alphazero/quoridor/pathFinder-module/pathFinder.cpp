...
bitset<128> find_path_and_non_blocking_walls(int x, int y, int x_target, int wall, int* block_) {
	...
	vector<int> block(block_, block_ + (selfn * selfn));
	...
	if (wall < (num_walls/2)) {
		wx = (wall / selfn_)*2 + 1;
		wy = (wall % selfn_)*2;
		block[wx * selfn + wy + 2] = 1;
	} else {
		wx = ((wall-(num_walls/2)) / selfn_)*2 + 2;
		wy = ((wall-(num_walls/2)) % selfn_)*2 + 1;
		block[(wx - 2) * selfn + wy] = 1;
	}
	block[wx * selfn + wy] = 1;
	// ============================

	vector<int> parent(selfn * selfn * 2, 0);
	parent[(x * selfn + y) * 2] = -1;
	parent[(x * selfn + y) * 2 + 1] = -1;

	queue <pair<int,int> > next;
	vector<int> visited(selfn * selfn, 0);
	add_next_visits(next, x, y, x_target, block.data(), visited.data(), parent.data());
	while (!next.empty()) {
		pair<int, int> tmp = next.front(); next.pop();
		if (tmp.first == x_target) // found a path
			return walls_not_in_path(tmp.first, tmp.second, parent.data());

		add_next_visits(next, tmp.first, tmp.second, x_target, block.data(), visited.data(), parent.data());
	}
...
static PyObject* legalWalls(PyObject* self, PyObject* args)
{
	...
	vector<int> block(selfn * selfn, 0);
	...
	bitset<128> all_walls = possible_walls(block.data());
	...
	while (++next_try < num_walls) {
		...
		legal_walls |= find_path_and_non_blocking_walls(x, y, 0, next_try, block.data()) & find_path_and_non_blocking_walls(x_, y_, selfn - 1, next_try, block.data()) & all_walls;
